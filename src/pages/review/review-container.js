...
function Review(props) {
  ...
  const handleUserAnswer = useCallback(payload => {
  ...
  const handleRevealButtonClick = useCallback(() => dispatch({type: 'setRevealed'}), []);

  const handleAnswerButtonMouseOver = useCallback(buttonType => {
    const setSwiperPositionActionType = buttonType === 'correct'
      ? 'setCorrectSwiperPosition'
      : 'setIncorrectSwiperPosition';
    state.isUILocked || dispatch({
      type: setSwiperPositionActionType,
      payload: {
        position: 'peek'
      }
    });
  ...
  const handleAnswerButtonMouseOut = useCallback(buttonType => {
    const setSwiperPositionActionType = buttonType === 'correct'
      ? 'setCorrectSwiperPosition'
      : 'setIncorrectSwiperPosition';
    state.isUILocked || dispatch({
  ...
  const handleAnswerButtonClick = useCallback(buttonType => {
    state.isUILocked || handleUserAnswer({type: buttonType})
  }, [handleUserAnswer, state.isUILocked])

  const answerButtonsInfo = [
    {
      type: 'incorrect',
      icon: 'x',
      label: 'Incorrect'
  ...
  return <ReviewView progressHistory={props.progressHistory} {...{currentFlashcard, currentOpenModal, modalExitFunction, state, answerButtonsInfo, handleAnswerButtonMouseOver, handleAnswerButtonMouseOut, handleAnswerButtonClick, handleRevealButtonClick}}/>
}
